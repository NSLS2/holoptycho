...
# ---------------------------------------------------------------------------
# Inline D4 transforms (no ptychoml import needed)
# ---------------------------------------------------------------------------
...
def _apply_d4(arr, name):
    if name not in _D4:
        raise ValueError(f"Unknown D4 transform {name!r}; valid: {list(_D4)}")
    return _D4[name](arr)
...
def dump(
    ...
    with h5py.File(output_path, "w") as f:
        ...
        for i in range(0, n_frames, chunk_size):
            ...
            chunk = np.ascontiguousarray(_apply_d4(chunk, dp_orient))

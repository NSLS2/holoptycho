...
# Restart with the same config (use after updating config for a new scan)
hp restart
...
# 4. Start the pipeline (ZMQ env vars must be set in the container)
hp start
...
# 7. For the next scan: update the config and restart
hp config set scan320046 '{ ... }'
hp config select scan320046
hp restart
```
